...
export default async function AgentPartnerPage() {
  ...
  return (
    <main className="bg-white">
      ...
      {/* ── Hero ── */}
      <PageHero
        gradient="linear-gradient(130deg,#1249C4 0%,#1A5CE8 50%,#5A8CFF 100%)"
        breadcrumbLabel="Agents & Franchisees"
        eyebrow={s.badge}
        eyebrowColor="#CCFF00"
        heading={renderHeading(s.heading)}
        paragraph={s.subheading}
        image={{ src: 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?w=1200&q=80&auto=format&fit=crop', alt: 'Education consultant reviewing student application documents at a desk' }}
        badge={{ value: s.stats[0]?.value ?? '400+', label: s.stats[0]?.label ?? 'Direct Tie-Ups' }}
        blobColor="#CCFF00"
      />
