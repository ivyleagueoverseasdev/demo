...
function NavLinks({
  ...
  const active = 'text-amber-300 bg-white/[0.12]';
  const idle   = 'text-white hover:text-white hover:bg-white/10';
  ...
  return (
    <>
      ...
      >
        ...
        >
          <div className="container-xl flex items-center justify-between h-9 text-white text-[11.5px] font-jakarta border-b border-white/[0.07]">
            ...
            <div className="hidden md:flex items-center gap-4">
              <span className="text-white/70 font-medium">Trusted by 10,000+ Students</span>
              <span className="w-px h-3 bg-white/20" />
              <a href={COMPANY.wa} target="_blank" rel="noopener noreferrer"
                className="flex items-center gap-1.5 text-white/85 hover:text-white transition-colors font-medium">
          ...
          >
            ...
            <Link href="/" aria-label="ILOC" className="flex-shrink-0">
              ...
              >
                ...
                >
                  <Image
                    src="/logo2.png"
                    alt="Ivy League Overseas Consulting"
                    fill
            ...
            <Link href="/" className="flex-shrink-0 min-w-0">
              <motion.div
                ...
                whileHover={{
                  textShadow: '0 0 28px rgba(255,255,255,0.30)',
              ...
              >
                <motion.div
                  ...
                  transition={{ duration: 0.32, ease: 'easeInOut' }}
                  className="font-jakarta font-black text-white leading-tight tracking-tight"
                  style={{ textShadow: '0 2px 20px rgba(0,0,0,0.20)' }}
                >
            ...
            <AnimatePresence>
              {scrolled && (
                ...
                >
                  <a href={COMPANY.wa} target="_blank" rel="noopener noreferrer"
                    className="font-jakarta font-medium text-[13px] text-white/80 hover:text-white transition-colors px-3 py-2">
                    💬 WhatsApp
                  </a>
                  <Link href="/contact"
            ...
            >
              <div className="w-5 space-y-1.5">
                <motion.span animate={{ rotate: mOpen ? 45 : 0, y: mOpen ? 7 : 0 }}
                  className="h-0.5 bg-white block rounded-full origin-center" />
        ...
        >
          <div
            className="container-xl flex items-center justify-between border-t border-white/[0.07]"
            style={{ height: 58 }}
          >
            <NavLinks
              isActive={isActive}
            ...
            <div className="hidden lg:flex items-center gap-2 ml-auto">
              <a href={COMPANY.wa} target="_blank" rel="noopener noreferrer"
                className="font-jakarta font-medium text-sm text-white/80 hover:text-white transition-colors px-3 py-2">
                💬 WhatsApp
              </a>
              <Link href="/contact"
          ...
          >
            <div className="container-xl py-4 flex flex-col gap-1">
              {NAV.filter(n => n.href !== '/destinations' && n.href !== '/partners').map(({ label, href }) => (
                <Link key={href} href={href}
                  className={`font-jakarta font-medium text-sm px-4 py-3 rounded-xl transition-colors ${
                    isActive(href) ? 'text-amber-300 bg-white/[0.12]' : 'text-white hover:bg-white/10'
                  }`}>
                  {label}
                </Link>
              ))}
              ...
              <Link href="/destinations"
                className="px-4 pb-1 font-jakarta text-[10px] font-bold text-white/50 uppercase tracking-wider hover:text-white/80 transition-colors">
                Destinations →
              </Link>
              <div className="grid grid-cols-2 gap-1 px-1">
                {DESTINATIONS.map(d => (
                  <Link key={d.href} href={d.href}
                    className="font-jakarta font-medium text-xs px-3 py-2 rounded-lg text-white/80 hover:bg-white/10 hover:text-white transition-colors">
                    {d.label}
                  </Link>
                ))}
              </div>
              ...
              <p className="text-center py-2 text-xs text-white/40 font-jakarta">📞 {COMPANY.phone}</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
